...
class Window:
    ...
    def create_menu_bar(self, quiz):
        ...
        # add menu items to the File menu
        preference_menu.add_command(
            label='Switch theme', command=lambda q=quiz: self.switch_theme(q))
    ...
    def get_user_input(self, quiz):
        ...
        no_of_question_combobox = ttk.Combobox(
            all_info, state="readonly", textvariable=no_of_questions)
        ...
        t_per_question_combobox = ttk.Combobox(
            all_info,  state="readonly", textvariable=t_per_question)
        t_per_question_combobox['values'] = [
        ...
        rb1 = ttk.Radiobutton(all_info, text='Light theme',
                              value=False, variable=selected_theme)
        rb2 = ttk.Radiobutton(all_info, text='Dark theme',
                              value=True, variable=selected_theme)
...
class Quiz:
    def __init__(self):
        ...
        self.all_data = pd.read_json("questions/all_questions.json")
        ...
        self.possible_questions_id = [
            x for x in range(len(self.questions_info))]
    ...
    def next_question(self):
        ...
        for k, v in self.current_question.choices.items():
            ...
            self.buttons[count]['command'] = lambda t=k, button_id=count: self.select(
                button_id, t, button_id_answer)
    ...
    def end_quiz(self):
        ...
        final_score += f'Correct: {self.correct}/{
            self.question_no}\nWrong: {self.wrong}/{self.question_no}\n'
        final_score += f'Questions answered: {
            self.question_no}/{self.number_of_questions}\n'
        if percentage > 70:
    ...
    def timer(self):
        ...
        while total_time_seconds > 0:
            if not self.pause:
                self.label_timer['text'] = f"{hours:02d}:{
